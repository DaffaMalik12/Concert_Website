...
// Check if form submitted for adding new admin
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_admin'])) {
    // Mengambil data dari formulir
    $username = $_POST['username'];
    $email = $_POST['email'];
    $password = $_POST['password']; // Anda harus mengenkripsi password sebelum menyimpannya di database (misalnya dengan password_hash)

    // Query untuk menambah admin baru ke tabel_admin
    $sql = "INSERT INTO admin_table (username, email, password) VALUES ('$username', '$email', '$password')";
...
// Check if form submitted for updating admin
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_admin'])) {
    ...
    // Query untuk mengedit admin di tabel_admin
    $sql = "UPDATE tabel_admin SET username='$username', email='$email', password='$password' WHERE id='$id'";
...
// Check if ID parameter exists for deleting admin
if (isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == 'delete') {
    // Mengambil ID dari parameter URL
    $id = $_GET['id'];

    // Query untuk menghapus admin dari tabel_admin
...
// Query untuk mengambil data admin dari tabel_admin
                    ...
                    <li class="relative px-6 py-3">
                        ...
                        <template x-if="isPagesMenuOpen">
                            <ul x-transition:enter="transition-all ease-in-out duration-300" x-transition:enter-start="opacity-25 max-h-0" x-transition:enter-end="opacity-100 max-h-xl" x-transition:leave="transition-all ease-in-out duration-300" x-transition:leave-start="opacity-100 max-h-xl" x-transition:leave-end="opacity-0 max-h-0" class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900" aria-label="submenu">
                                ...
                                <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
                                    <a class="w-full" href="./blank.html">Akun Admin</a>
                                </li>
                                <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
                                    <a class="w-full" href="./blank.html">Akun User</a>
                                </li>
                            </ul>
                        </template>
                    </li>
